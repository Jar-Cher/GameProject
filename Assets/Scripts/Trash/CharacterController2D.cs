...
public class CharacterController2D : MonoBehaviour {
	...
	IEnumerator KillPlayer()
	{
		if (playerCanMove)
		{
			...
			if (GameManagerArcived.gm) // if the gameManager is available, tell it to reset the game
				GameManagerArcived.gm.ResetGame();
	...
	public void CollectCoin(int amount) {
		...
		if (GameManagerArcived.gm) // add the points through the game manager, if it is available
			GameManagerArcived.gm.AddPoints(amount);
	}
	...
	public void Victory() {
		...
		if (GameManagerArcived.gm) // do the game manager level compete stuff, if it is available
			GameManagerArcived.gm.LevelCompete();
	}
